...
export function BlueprintModal({ open, artifacts, onClose }: Props) {
  ...
  return <AnimatePresence>{open && <motion.div className="modal-wrap" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} role="dialog" aria-modal="true" aria-label="Revival Blueprint">
    ...
    <motion.div className="blueprint" initial={{y:70, scale:.97}} animate={{y:0, scale:1}} exit={{y:40, opacity:0}} transition={{duration:.6, ease:[.22,1,.36,1]}}>
      ...
      {artifacts.length > 1 && <div className="blueprint-specimens">{artifacts.map(a => <span key={a.id}><i style={{background:a.exhibit.accent}}/>{a.name}</span>)}</div>}
      ...
      <div className="prompt-preview"><p>BUILD PROMPT PREVIEW</p><code>{single ? `以「${single.name}」经过核验的 Product DNA 为起点，构建 clean-room 2026 MVP；保留可验证价值，删除未经证实的历史假设，并解释 AI 改变了哪些原始约束……` : artifacts.length > 1 ? `对照 ${artifacts.map(a=>a.name).join('、')} 的可验证 Product DNA，找出共同需求、互补机制与仍然存在的硬约束；只在证据支持时组合，并生成最小可验证 2026 MVP……` : '以可验证的 Product DNA 为起点，构建 clean-room 2026 MVP……'}</code></div>
    </motion.div>
